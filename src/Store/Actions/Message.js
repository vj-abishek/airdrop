...
export const sendmessage = (message, details) => async (dispatch, getState) => {
    ...
    const [to] = getState()
        .channelReducer
        .channels
        .filter((id) => id.channelId === details.channel);
...
export const SyncMessages = (channelId) => async (dispatch, getState) => {
    ...
    try {
        const c = await db.message
            .where('channel').equals(channelId).count();
        console.log(c);
...
export const TypingIndication = (status) => (dispatch, getState) => {
    ...
    const [to] = getState()
        .channelReducer
        .channels
        .filter((id) => id.channelId === status.channel);
    ...
    socket.on('new message', (message) => {
        message.forEach(async (msg) => {
            try {
                ...
                dispatch({ type: 'ON_MESSAGE', payload: { channel: msg.channel, messages: final } });
                await db.message.add(final);
